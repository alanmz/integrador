  ...
  */
  $.each(questions, function(question_index, question) {
    ...
    $.each(question.answers, function(answer_index, answer) {
      ...
      if (last_question) {
        opts.confirmButtonText = "Ver tus Resultados";
      }
  ...
  var $social = $("<div>")
    ...
    .html('<div id = "social-text">Realizado por Alan Muñoz y Daniel Cstillo</div>')
    .appendTo($results_slide);
...
function resultsText(state) {
  ...
  switch (true) {
    case (ratio === 1):
      text = "Perfecto Todas las Respuestas Correctas!";
      break;
    case (ratio > 0.9):
      text = "Excelente Trabajo La mayoria estuvieron correctas.";
      break;
    case (ratio > 0.60):
      text = "Bien Hecho sigue asi.";
      break;
    case (ratio > 0.5):
      text = "Regular sigue tratando";
      break;
    case (ratio < 0.5 && ratio !== 0):
      text = "Malo La mayoria de respuestas equivocadas.";
      break;
    case (ratio === 0):
      text = "Ninguna Correcta";
      break;
  }
  return text;
